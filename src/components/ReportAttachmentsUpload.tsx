...
const ReportAttachmentsUpload = ({ formik }: ReportAttachmentsUploadProps) => {
  ...
  const handleFileSelection = async (files: FileList) => {
    ...
    if (attachments.length + newFiles.length > MAX_FILES) {
      toast({ title: `Maximum ${MAX_FILES} attachments allowed` });
      return;
    }
    ...
    await Promise.all(
      newAttachments.map(async (tempAttachment, index) => {
        ...
        if (uploadResult?.success) {
          ...
          toast({ title: `Failed to upload ${tempAttachment.name}` });
        }
      })
    );
    ...
    <div>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          Maximum {MAX_FILES} attachments (5.0 MB each)
          <br />
          Allowed file types (.pdf, .png, .jpg, .txt, .doc, .docx)
        </p>
      </div>
      ...
      {attachments.length > 0 && (
        <div className="mt-4 space-y-2">
          <h4 className="text-sm font-medium">
            Selected Files ({attachments.length}/{MAX_FILES})
          </h4>
