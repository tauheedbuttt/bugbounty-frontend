        ...
        <div>
          {comments.map((item, index) => {
            ...
            const isSameUser =
              ...
              next?.username === item.username;
            const showThread = !isLast && isSameUser;
            return (
              <>
                <div
                  key={item._id}
                  className={cn(
                    "thread-item border rounded-lg",
                    showThread ? "" : "mb-4"
                  )}
                >
                  <div className="flex items-start gap-3 ">
                    <div className=" h-full pt-4 pl-4 flex flex-col items-center">
                      ...
                      {showThread && (
                        <div className="h-8 w-[1px] bg-muted-foreground" />
                      )}
                    </div>
                ...
                {showThread && (
                  <div className="h-4 w-[1px] bg-muted-foreground ml-[32.5px]" />
                )}
              </>
            );
          })}
